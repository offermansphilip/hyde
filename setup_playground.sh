...
# Check if requirements_playground.txt exists
if [ -f "requirements_playground.txt" ]; then
    echo "Installing required Python modules from requirements_playground.txt..."
    pip install -r requirements_playground.txt
else
    echo "requirements_playground.txt not found. Please ensure it is in the current directory."
fi
